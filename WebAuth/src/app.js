...
Amplify.configure({
    Auth: {
        ...
        userPoolId: 'eu-west-1_PM7q7l7wr',

        // OPTIONAL - Amazon Cognito Web Client ID (26-char alphanumeric string)
        userPoolWebClientId: '4jrfgpiq1iqir7j2ltqmugtkcq',
...
RequestToSecuredEndpointButton.addEventListener('click', async evt => {
  ...
  await API.get("MyAPIGatewayAPI", '/dev/api/getTogethers', myInit).then(response => {
...
RequestToNonSecuredEndpointButton.addEventListener('click', async evt => {
  await API.get("MyAPIGatewayAPI", '/dev/api/testAutoSubscribe').then(response => {
    console.log(response);
  }).catch(error => {
      console.log(JSON.stringify(error))
  });
